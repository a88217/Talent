...
RUN --mount=type=secret,id=private_pem,dst=/etc/secrets/private.pem cp /etc/secrets/private.pem

RUN --mount=type=secret,id=public_pem,dst=/etc/secrets/public.pem cp /etc/secrets/public.pem
